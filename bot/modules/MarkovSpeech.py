...
class MarkovSpeech(BaseModule):
    ...
    def generate_sentence(self,msg):
        ...
        while len(sentence)<350:
          if i == 0 and not len(sentence):
            ...
            word = self.get_word(sentence[i-1])

          if not word or word[-1].strip().endswith(('.', '!', '?')):
            break
          else:
    ...
    def get_word(self,wrd, first=0):
      ...
      if first or not wrd:
        cursor.execute('select RANDOM()*(occurance*0.01) as choice,w1.word,w2.word as wrd,occurance from sequence join word as w1 on w1.id=first join word as w2 on w2.id=second WHERE first_word=1 ORDER BY choice ASC LIMIT 1;')
      elif wrd and not first:
        cursor.execute("select RANDOM()*(occurance*0.01) as choice,w1.word,w2.word as wrd,occurance from sequence join word as w1 on w1.id=first join word as w2 on w2.id=second WHERE w1.word=? ORDER BY choice ASC LIMIT 1;",(wrd,))
      else:
        cursor.execute("select RANDOM()*(occurance*0.01) as choice,w1.word,w2.word as wrd,occurance from sequence join word as w1 on w1.id=first join word as w2 on w2.id=second WHERE w1.word=? ORDER BY choice ASC LIMIT 1;",(wrd,))
